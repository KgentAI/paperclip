...
function formatLastRunTimestamp(value: Date | string | null | undefined, t: (key: string) => string) {
  if (!value) return t("routines.never");
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(isArchived || routine.status === "paused" || isDraft) ? (
            <span className="text-xs text-muted-foreground">
              {isArchived ? t("routines.status.archived") : isDraft ? t("routines.status.draft") : t("routines.status.paused")}
            </span>
          ) : null}
        </div>
      ...
      <div className="flex items-center gap-3" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
        <div className="flex items-center gap-3">
          ...
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? t("routines.status.archived") : isDraft ? t("routines.status.draft") : enabled ? t("routines.status.on") : t("routines.status.off")}
          </span>
        </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem asChild>
              <Link to={href}>{t("common.action.edit")}</Link>
            </DropdownMenuItem>
            <DropdownMenuItem
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        ...
        body: routine.assigneeAgentId
          ? t("routines.addTriggerMessage")
          : t("routines.draftSavedMessage"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines.couldNotUpdate"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines.couldNotStart"),
        tone: "error",
      });
    },
  });
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {(routines ?? []).length} {t("routines.routineCount", { count: (routines ?? []).length })}
            </p>
            <Popover>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["project", t("routines.groupByProject")],
                    ["assignee", t("routines.groupByAgent")],
                    ["none", t("routines.groupByNone")],
        ...
        >
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("routines.afterCreationMessage")}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("common.action.creating") : t("routines.createRoutine")}
              </Button>
