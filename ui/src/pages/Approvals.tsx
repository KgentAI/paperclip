...
export function Approvals() {
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("approvals.errors.approveFailed"));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("approvals.errors.rejectFailed"));
    },
  });
  ...
  if (!selectedCompanyId) {
    return <p className="text-sm text-muted-foreground">{t("approvals.empty.noCompany")}</p>;
  }
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            {statusFilter === "pending" ? t("approvals.empty.noPending") : t("approvals.empty.noApprovals")}
          </p>
        </div>
      )}
