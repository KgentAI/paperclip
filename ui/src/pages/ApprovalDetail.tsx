...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("approvals.approvalDetail") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals.errors.approveFailed")),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals.errors.rejectFailed")),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals.errors.revisionFailed")),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals.errors.resubmitFailed")),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals.errors.commentFailed")),
  });
  ...
  if (!approval) return <p className="text-sm text-muted-foreground">{t("common.error.notFound")}</p>;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approvals.approvalConfirmedDescription")}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("approvals.decisionNote")}: {approval.decisionNote}</p>
          )}
        </div>
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("approvals.linkedIssuesDescription")}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approvals.resolveBudgetStop")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("approvals.deleteAgentConfirm"))) return;
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("comments.title")} ({comments?.length ?? 0})</h3>
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity name={t("common.board")} size="sm" />
                )}
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? t("common.posting") : t("comments.send")}
          </Button>
        </div>
      </div>
    </div>
