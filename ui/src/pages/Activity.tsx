...
export function Activity() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={History} message={t("activity.empty.noCompany")} />;
  }
  ...
  return (
    <div className="space-y-4">
          ...
          <SelectContent>
            ...
            {entityTypes.map((type) => (
              <SelectItem key={type} value={type}>
                {t(`activity.entityType.${type}`, { defaultValue: type.charAt(0).toUpperCase() + type.slice(1) })}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      {filtered && filtered.length === 0 && (
        <EmptyState icon={History} message={t("activity.empty.noActivity")} />
      )}
