  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("common.label.status")}>
          <StatusBadge status={agent.status} />
        </PropertyRow>
        <PropertyRow label={t("common.label.type")}>
          <span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span>
        </PropertyRow>
        {agent.title && (
          <PropertyRow label={t("agents.agentTitle")}>
            <span className="text-sm">{agent.title}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("agents.config.adapter")}>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t("common.label.description")}>
        ...
        <PropertyRow label={t("common.label.date")}>
