...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: (key: string) => string) {
  return windowKind === "lifetime" ? t("costs.budgetPolicy") : t("costs.budgetLimit");
}
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("costs.softThreshold")}</div>
      ...
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("costs.hardThreshold")}</div>
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("common.action.save") : summary.amount > 0 ? t("common.action.save") : t("common.action.save")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("agents.status.paused") : summary.status === "warning" ? t("issues.status.inReview") : summary.status === "hard_stop" ? t("costs.hardThreshold") : t("agents.status.active")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("common.error.generic")}</p>
        ) : null}
      </div>
    );
  }
