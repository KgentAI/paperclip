  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? `${t("common.action.filter")}: ${activeFilterCount}` : t("common.action.filter")) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{activeFilterCount > 0 ? `${t("common.action.filter")}: ${activeFilterCount}` : t("common.action.filter")}</span>}
      ...
      >
        <div className="space-y-3 p-3">
          ...
          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("common.action.filter")}</span>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issues.filter.status")}</span>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issues.filter.priority")}</span>
                <div className="space-y-0.5">
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("issues.filter.assignee")}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  ...
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{t("sidebar.account.profile")}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issues.filter.creator")}</span>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("issues.filter.searchCreators")}
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("common.action.noResults")}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issues.filter.project")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("issues.filter.label")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {labels.map((label) => (
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("workspaces.title")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
