                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {item.kind} · {item.statusLabel === "stopped" ? t("workspaces.runtime.stopped") : item.statusLabel === "starting" ? t("workspaces.runtime.starting") : item.statusLabel === "running" ? t("workspaces.runtime.running") : item.statusLabel === "run once" ? t("workspaces.runtime.runOnce") : item.statusLabel === "stopping" ? t("workspaces.runtime.stopping") : item.statusLabel}
...
export function WorkspaceRuntimeControls({
  ...
  const resolvedServiceEmptyMessage = emptyMessage ?? serviceEmptyMessage ?? t("workspaces.noServicesConfigured");
  const resolvedJobEmptyMessage = jobEmptyMessage ?? t("workspaces.noJobsConfigured");
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className="rounded-xl border border-border/70 bg-background/60 p-3">
        <div className="space-y-1">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces.workspaceCommands")}</div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0 ? t("workspaces.servicesRunning", { count: runningCount }) : t("workspaces.noServicesRunning")}
            </span>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ? t("workspaces.jobsAvailable", { count: resolvedSections.jobs.length })
                : t("workspaces.commandsControlledIndependently")}
            </span>
          </div>
      ...
      <CommandSection
        title={t("workspaces.services")}
        description={t("workspaces.servicesDescription")}
      ...
      <CommandSection
        title={t("workspaces.jobs")}
        description={t("workspaces.jobsDescription")}
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          title={t("workspaces.untrackedServices")}
          description={t("workspaces.untrackedServicesDescription")}
