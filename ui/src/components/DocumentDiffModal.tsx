...
function getRevisionLabel(revision: DocumentRevision, t: (key: string) => string) {
  const actor = revision.createdByUserId
    ? t("common.label.board")
    : revision.createdByAgentId
      ? t("common.label.agent")
      ...
      <DialogContent className="!max-w-[90%] w-full max-h-[85vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between gap-4">
          <DialogHeader className="shrink-0">
            <DialogTitle>
              {t("common.action.diff")} — <span className="font-mono text-sm">{documentKey}</span>
            </DialogTitle>
          </DialogHeader>

          <div className="flex items-center gap-4 shrink-0">
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-red-400">{t("common.label.old")}</span>
              <Select
              ...
              >
                <SelectTrigger className="h-7 w-60 text-xs border-border/60">
                  <SelectValue placeholder={t("common.action.selectRevision")} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-green-500/30 bg-green-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-green-400">{t("common.label.new")}</span>
              <Select
        ...
        <div className="overflow-auto flex-1 rounded-md border border-border text-xs">
          {!revisions ? (
            ...
            <div className="p-6 text-center text-muted-foreground text-sm">{t("diff.selectTwoRevisions")}</div>
          ) : leftRevision.id === rightRevision.id ? (
            <div className="p-6 text-center text-muted-foreground text-sm">{t("diff.sameRevision")}</div>
          ) : (
            <div className="font-mono text-[12px] leading-6">
              <div className="grid grid-cols-[56px_56px_24px_minmax(0,1fr)] border-b border-border/60 bg-muted/30 px-3 py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
                <span>{t("common.label.old")}</span>
                <span>{t("common.label.new")}</span>
                <span />
                <span>{t("common.label.content")}</span>
              </div>
